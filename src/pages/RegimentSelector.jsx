...
const createDefaultRegiments = (divisionDefinition, getRegimentDefinition) => {
    ...
    const baseRegiments = divisionDefinition.base.map((group, index) => createRegimentWithDefaults('base', index, group.options[0]));
    const additionalRegiments = divisionDefinition.additional.map((group, index) => ({ group: "additional", index, id: "none", name: null, config: {}, }));
    return { base: baseRegiments, additional: additionalRegiments, supportUnits: [], divisionDefinition: divisionDefinition };
};
...
export default function RegimentSelector({
    ...
    if (!configuredDivision) return <div style={{ padding: 20 }}>Ładowanie konfiguracji dywizji...</div>;
    const { base: baseRegiments, additional: additionalRegiments, supportUnits } = configuredDivision;

    const handleBuySupportUnit = (unitId) => {
        if (purchasedUnitIds.includes(unitId)) { alert(`Jednostka została już zakupiona.`); return; }

        // NEW: PU COST VALIDATION
        ...
        setConfiguredDivision(prev => ({ ...prev, supportUnits: [...prev.supportUnits, { id: unitId, assignedTo: null }] }));
    };

    const handleAssignSupportUnit = (unitIndex, positionKey) => {
        if (!positionKey) { handleUnassignSupportUnit(unitIndex); return; }
    ...
    const handleRegimentChange = (groupKey, index, newRegimentId) => {
        setConfiguredDivision((prev) => {
            ...
            if (def && def.structure && def.structure.base) {
                Object.entries(def.structure.base).forEach(([slotKey, options]) => {
                    const isOptional = slotKey.toLowerCase().startsWith('optional');
                    if (!isOptional && Array.isArray(options) && options.length > 0) config.base[slotKey] = options[0];
                    else config.base[slotKey] = null;
                });
            }
            newGroup[index] = { ...newGroup[index], id: newRegimentId, config: config };
            const positionKey = `${groupKey}/${index}`;
            if (newRegimentId === 'none') {
                newSupportUnits.forEach(su => {
                    if (su.assignedTo?.positionKey === positionKey) su.assignedTo = null;
                });
            }
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                            {!isNone && (
                                <div style={{ fontSize: 12, color: '#555', textAlign: 'right' }}>
                                    <div>Akt.: <strong>{finalActivations}</strong> | Zw.: <strong>{stats.recon}</strong> | Mot.: <strong>{finalMotivation}</strong></div>
                                    <div style={{ fontSize: 11, maxWidth: 400, whiteSpace: 'normal', lineHeight: '1.2em', marginTop: 4 }}>
                                        {stats.unitNames.join(', ')}
                                    </div>
                                </div>
                            )}
                            <div style={{ fontWeight: 700, fontSize: 16, color: stats.cost > 0 ? '#1b7e32' : '#666' }}>{stats.cost} pkt</div>
                        </div>
                    </div>

                    <select value={currentRegimentId} onChange={(e) => handleRegimentChange(groupKey, index, e.target.value)} style={{ padding: 8, minWidth: 250, marginRight: 15 }} disabled={isBase && options.length === 1}>
                        {options.map((optionId) => (
                            <option key={optionId} value={optionId}>{getRegimentName(optionId)}</option>
                        ))}
                    </select>

                    {canEdit && <button onClick={() => onOpenRegimentEditor(groupKey, index)} style={{ padding: '8px 12px', background: '#0077ff', color: 'white', border: 'none', borderRadius: 5, cursor: 'pointer' }}>Edytuj Pułk</button>}
                    <div style={{ marginTop: 10, fontSize: 12, color: '#0077ff' }}>
                        Przypisane wsparcie:
                        {supportUnits.filter(su => su.assignedTo?.positionKey === positionKey).length === 0
                            ? ' —'
                            : supportUnits.filter(su => su.assignedTo?.positionKey === positionKey).map(su => unitsMap[su.id]?.name || su.id).join(', ')
                        }
                    </div>
                </div>
            );
    ...
    return (
        <div>
            ...
            <div style={{ padding: 12, marginBottom: 20, border: '2px solid #0077ff', borderRadius: 8, background: '#e9f3ff' }}>
                ...
                <div style={{ fontSize: 14, marginTop: 5 }}>(Koszt bazowy Dywizji: {divisionBaseCost} pkt)</div>
                <div style={{ fontSize: 16, marginTop: 10, fontWeight: 700 }}>Punkty ulepszeń: <span style={{ color: impColor, marginLeft: 5 }}>{remainingImprovementPoints} / {improvementPointsLimit}</span>{remainingImprovementPoints < 0 && <span style={{ color: 'red', marginLeft: 10 }}>[PRZEKROCZONO LIMIT!]</span>}</div>
            </div>
            <div style={{ color: "#444", marginBottom: 20 }}>Frakcja: <strong>{faction.meta?.name ?? faction.meta?.key}</strong></div>
            <section style={{ marginBottom: 30 }}>
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
                    {additionalUnitsDefinitions.map((item, idx) => {
                        if (typeof item === 'string') {
                            ...
                            return (
                                <button key={unitId} onClick={() => handleBuySupportUnit(unitId)} disabled={isPurchased} style={{ padding: 10, border: `1px solid ${isPurchased ? '#aaa' : '#1b7e32'}`, background: isPurchased ? '#f0f0f0' : '#e6ffe6', borderRadius: 6, cursor: isPurchased ? 'not-allowed' : 'pointer', opacity: isPurchased ? 0.6 : 1 }}>
                                ...
                                <div key={idx} style={{ border: '1px dashed #999', padding: 8, borderRadius: 8, display: 'flex', flexDirection: 'column', gap: 5, alignItems: 'center' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 5 }}>
                                        {item.options.map(optId => {
                                            ...
                                            const costPU = unitDef?.pu_cost ? ` | ${unitDef.pu_cost} PU` : '';
                                            return (
                                                <button key={optId} onClick={() => handleBuySupportUnit(optId)} disabled={isLocked} style={{ padding: '8px 12px', fontSize: 12, border: `1px solid ${isThisPurchased ? '#aaa' : '#1b7e32'}`, background: isThisPurchased ? '#ddd' : (isLocked ? '#f0f0f0' : '#e6ffe6'), borderRadius: 4, cursor: isLocked ? 'not-allowed' : 'pointer', opacity: isLocked ? 0.5 : 1, minWidth: 80 }}>
                                                    {isThisPurchased ? 'POSIADANA' : '+ Kup'} <br/> {unitsMap[optId]?.name || optId} ({unitsMap[optId]?.cost || 0} pkt{costPU})
                                                </button>
                                            );
                                        })}
                                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 15 }}>
                    {supportUnits.map((su, index) => {
                        ...
                        const assignedRegimentName = assignedRegiment ? getRegimentName(assignedRegiment.id) : '— Nieprzypisana';
                        return (
                            <div key={index} style={{ padding: 12, border: '1px solid #ddd', borderRadius: 8, background: '#fff' }}>
                                ...
                                <div style={{ fontSize: 12, color: su.assignedTo ? '#0077ff' : '#aaa' }}>Przypisano do: <strong>{assignedRegimentName}</strong></div>
                                <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                                    <select value={su.assignedTo?.positionKey || ''} onChange={(e) => handleAssignSupportUnit(index, e.target.value)} style={{ padding: 6, flexGrow: 1 }}>
                                        <option value="">Wybierz Pułk...</option>
                                        {regimentsList.map(r => (
                                            <option key={r.positionKey} value={r.positionKey}>{r.positionKey.startsWith('base') ? 'P. Podstawowy ' : 'P. Dodatkowy '} {getRegimentName(r.id)}</option>
                                        ))}
                                    </select>
                                    <button onClick={() => handleSellSupportUnit(index)} style={{ padding: '6px 8px', background: '#ffe6b3', border: '1px solid orange', borderRadius: 4 }}>Sprzedaj</button>
                                </div>
                            </div>
                        );
                    })}
