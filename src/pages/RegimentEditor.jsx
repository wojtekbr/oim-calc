...
import {
    ...
    // WAŻNE: Import funkcji sprawdzającej darmowość konkretnego kliknięcia
    ...
    return (
        ...
        >
            ...
            <div className={styles.cardContent}>
                ...
                <div className={styles.multiCardTotalCost}>Razem: {totalCost} PS</div>
            </div>
        </div>
    );
};
...
const ActiveOptionConfigurationPanel = ({
                                            ...
                                            helpers,
                                            currentConfig // Config przekazany z góry
                                        }) => {
    return (
                        ...
                        {!isGroupRank && (
                            <div className={styles.cleanImprovementsContainer}>
                                {validImprovements.length > 0 ? validImprovements.map(imp => {
                                    ...
                                    const willBeFree = checkIfImprovementWouldBeFree(currentConfig, regiment, uid, imp.id);

                                    // 2. Koszt i "stać nas"
                                    // Jeśli będzie darmowe, to kosztuje nas efektywnie 0, więc stać nas zawsze
                                    ...
                                    // 3. Sprawdzamy limit ("płatny")
                                    const effectiveCount = calculateEffectiveImprovementCount(currentConfig, regiment, imp.id);
                                    const isLimitReached = imp.max_amount && effectiveCount >= imp.max_amount;

                                    // 4. Decyzja: Czy przycisk aktywny?
                                    // Można kliknąć, jeśli:
                                    // - Jest już wybrany (żeby odznaczyć)
                                    // - LUB (Limit nie osiągnięty ORAZ Stać nas na to)
                                    // - LUB (Limit osiągnięty ALE ten ruch jest darmowy i nie wlicza się do limitu)
                            ...
                            <div className={styles.tableContainer}>
                                ...
                                <table className={styles.impTable}>
                                    ...
                                    {definitions.unitLevelImprovements.map(imp => {
                                        ...
                                        const effectiveCount = calculateEffectiveImprovementCount(tempConfig, regiment, imp.id);
